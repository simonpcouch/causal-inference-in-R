...
# is augmented with predictions and propensity weights for
# each element of the analysis set. this serves as the "bridge" between
# two calls to `fit_resamples()` (or `tune_*()`) in a causal workflow, the
...
augment_resamples <- function(tune_results, resamples, wt_fn) {
  collect_extracts(tune_results) %>%
  right_join(resamples, by = "id") %>%
  rowwise() %>%
  mutate(
    res = list(
      analysis(splits) %>%
      bind_cols(., predict(.extracts, ., type = "prob")) %>%
      mutate(wts = importance_weights(wt_fn(.pred_TRUE, net, .treated = "TRUE"))) %>%
...
propensity_model <- propensity_wf %>%
  fit(data = net_data)
...
Let's create the bootstrapped dataset with rsample's `bootstraps()` function.
...
fit_ipw_not_quite_rightly <-
  fit_resamples(
    outcome_wf,
    resamples = bootstrapped_net_data,
...
ipw_results <-
  ...
  augment_resamples(bootstrapped_net_data, wt_ate) %>%
  fit_resamples(
...
bind_rows(ipw_results$.extracts) |>
  filter(term == "netTRUE") |>
  ggplot(aes(estimate)) +
...
boot_estimate <- ipw_results |>
  structure(class = class(bootstrapped_net_data)) %>%
